...
private:
    ...
    string _name;
      ...
      CQueue<CHistoricalFix *> *_historicalFixes;
...
CFix::CFix(string name, int maxHistoricalFixesToShow, color clr, ENUM_LINE_STYLE style)
  ...
  _name = name;
         ...
         _historicalFixes = new CQueue<CHistoricalFix *>(maxHistoricalFixesToShow);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
CFix::~CFix()
  ...
  for (int i = _historicalFixes.Count(); i > 0; i--)
         {
            CHistoricalFix *historicalFix = _historicalFixes.Dequeue();
            delete historicalFix;
         }
    ...
    return (int)((time - (int)time) * 100);
      ...
      void CFix::Handle(datetime time, double price)
      {
         if (IsInRange(time))
         {
            ...
            _historicalFixes.Enqueue(historicalFix);
            
            if (_historicalFixes.Count() > _maxHistoricalFixesToShow)
            {
               delete _historicalFixes.Dequeue();
            }
         }
      }
